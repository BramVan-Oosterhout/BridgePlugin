...
sub renderAuction {
  ...
  ( $i, $out ) = $this->{theAuctionRounds} eq 'practice' ?  
...
sub renderAuctionRounds {
  ...
  my $auctionRounds = $this->{theAuctionRounds} ? $this->{theAuctionRounds} : $#bids/4;
  my $seat = 0;
  for ( $seat = 0; $seat/4 < $auctionRounds && $seat <= $#bids; ) {
    ($i, $out, $seat ) = $this->renderAuctionRound( $i, $out, $seat, @bids );
  }  
  return $i, $out;
}
...
sub renderAuctionPractice {
  ...
  if ( $this->{practiceAuctionRound} ) {
    for ( ; $par <= $this->{practiceAuctionRound} && $seat <= $#bids; $par++ ) {
      ($i, $out, $seat ) = $this->renderAuctionRound( $i, $out, $seat, @bids );
    }
  } 
...
{
  no warnings qw(uninitialized);; # @deal may have unitialised values.
...
sub formatPlay {
  ...
  return "This deal (" . 
        ...
        ") is incomplete (" . 
        length($deal) . 
        "). You cannot play this board." if length( $deal ) < 69;
...
sub printHand {
  ...
  my $out;
  foreach my $suit( @{$this->{orderedSuits}} ) {
    $out .= $this->{symbols}{substr($suit,0,1)} . ' ' . $cards{$seat}{$suit} . '<br />';
  }
  return $out;
}
...
sub parseParams {
  ...
  throw Error::Simple("Web: ".$this->{theDataWeb}." does not exist")
      unless $this->{theDataWeb} eq '' || Foswiki::Func::webExists( $this->{theDataWeb} );
      
  ($this->{theDataWeb}, $this->{theDataTopic}) = 
        Foswiki::Func::normalizeWebTopicName($this->{theDataWeb}, $this->{theDataTopic});
